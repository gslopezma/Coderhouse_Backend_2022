...
test('[GET] api/productos/:non-existent-id returns Producto no encontrado', async() => {
    ...
    expect(response.data).toHaveProperty('Producto no encontrado')

})

test('[PUT] api/productos/:non-existent-id returns Producto no encontrado', async() => {
    const nonExistentProductUrl = 'http://localhost:3031/api/productos/non-existent-id'
    const body = {}
